...
(() => {
  ...
  style.textContent = `
    ...
    .flash-stat { padding:14px; border:1px solid var(--line); border-radius:14px; background:#fff; }.flash-stat span { display:block; color:var(--muted); font-size:10px; font-weight:800; letter-spacing:.08em; text-transform:uppercase; }
    .flash-progress { height:8px; overflow:hidden; margin-bottom:20px; border-radius:99px; background:#e5eaf4; }.flash-progress i { display:block; width:0; height:100%; background:linear-gradient(90deg,var(--primary),#7b8cff); transition:width .2s ease; }
    .flash-card { min-height:350px; padding:clamp(25px,5vw,46px); border:1px solid var(--line); border-radius:20px; background:#fff; }.flash-category { color:var(--primary); font-size:11px; font-weight:800; letter-spacing:.1em; text-transform:uppercase; }
    .flash-word { margin-top:10px; font-size:clamp(34px,5vw,54px); font-weight:800; line-height:1.05; letter-spacing:-.05em; }
    .flash-ipa { margin-top:12px; color:var(--muted); font-family:"DM Mono", monospace; font-size:15px; }
    .flash-meaning { margin-top:22px; color:#1f2942; font-size:clamp(20px,3vw,28px); font-weight:700; line-height:1.35; }
    .flash-meaning.is-hidden { filter:blur(8px); user-select:none; }
    .flash-actions { display:grid; grid-template-columns:repeat(2,1fr); gap:8px; padding-top:20px; border-top:1px solid var(--line); }.flash-actions button { border-color:#dfe5f1; background:#fff; }.flash-actions button:last-child { grid-column:span 2; }
  ...
  document.body.querySelector('#add').insertAdjacentHTML('afterend', `
    <div id="flashcards" class="tab-content">
      ...
      <section class="flash-card"><div id="fcCategory" class="flash-category">Vocabulary</div><div id="fcWord" class="flash-word">—</div><div id="fcIpa" class="flash-ipa"></div><div id="fcMeaning" class="flash-meaning is-hidden"></div></section>
      <div class="flash-actions"><button onclick="flashReveal()">Hiện nghĩa</button><button onclick="flashAgain()">Chưa nhớ</button><button onclick="flashKnown()">Đã nhớ</button><button onclick="flashShuffle()">Trộn thẻ</button><button onclick="flashNext()">Thẻ tiếp theo</button></div>
    </div>`);
  ...
  function flashStats() { fcSeen.textContent = seenWords.size; fcKnown.textContent = knownWords.size; fcTotal.textContent = flashcards.length; fcCount.textContent = `${flashIndex + 1} / ${flashcards.length}`; fcBar.style.width = `${flashcards.length ? ((flashIndex + 1) / flashcards.length) * 100 : 0}%`; }
